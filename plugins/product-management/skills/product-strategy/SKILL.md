...
## Когда применять
- Формулируешь видение/позиционирование/roadmap, готовишь стратегию к Board/ExCo/команде.
- Переоцениваешь PMF/рынок/сегмент или реагируешь на конкурентный ход.
- Приоритизируешь портфель фич/тем, выравниваешь метрики и GTM.

## Подробные плейбуки (3 итерации)
### Итерация 1 — Диагностика (1–2 часа)
- Сбор фактов: NSM/цели, персоны/JTBD, рынок/конкуренты, ограничения/риски.
- Анализ текущего продукта: usage, retention, win/loss, отзывы, баги/качество.
- Артефакт: черновой бриф + риск/decision log.
### Итерация 2 — Дизайн (2–4 часа)
- 2–3 стратегических варианта и позиционирование; сценарии roadmap (base/upside/downside).
- Приоритизация (RICE/WSJF), Kano/JTBD/HEART/AARRR, KPI tree (NSM → драйверы).
- Риски/mitigation, GTM набросок, ресурсы/ограничения.
- Артефакт: таблицы вариантов, KPI tree, roadmap варианты.
### Итерация 3 — Верификация (1–2 часа)
- Пилоты/эксперименты, критерии go/no-go, алерты/стоп-пороги.
- План коммуникаций (команда/стейкхолдеры/leadership), частота обзоров.
- Обновление decision/risk log, TODO с владельцами/датами, фиксация изменений.

## Метрики и алерты
- **Ценность/рост:** NSM, AARRR, adoption, activation, retention (D1/D7/D30), win rate.
- **Экономика:** CAC/LTV, payback, ARPU/ARPA, маржа фич, cost-to-serve.
- **Качество/опыт:** HEART, баги/дефекты, отказоустойчивость, NPS/CSAT, time-to-value.
- **Конкуренты:** прайс/фича-паритет, доля/победы-сделок, скорость релизов vs топ-5.
- Алерты: падение NSM/retention, рост отказов/багов, отставание фич/прайса, burn/маржа фич.

## Входы (собери до старта)
- Цели/NSM, персоны/JTBD, данные usage/retention, win/loss, отзывы, баги/качество.
- Рынок/конкуренты (релизы, прайс, отзывы), ограничения (ресурсы/легал/данные/сроки).
- Текущий roadmap/долг/зависимости, GTM/каналы и их эффективность.

## Выходы (обязательно зафиксировать)
- Стратегический бриф + выбранный сценарий; KPI tree, roadmap сценарии, приоритизация (RICE/WSJF).
- План экспериментов/пилотов с порогами go/stop, алерты и владельцы.
- Decision/Risk logs, список TODO с датами и ответственными, изменения vs прошлой версии.

## Источники данных и инструменты
- Продуктовая аналитика, CRM/win-loss, NPS/CSAT, баг/качество, WebSearch/WebFetch по рынку/конкурентам.
- Фреймворки: JTBD/Kano/HEART/AARRR/RICE/WSJF/Porter/3C/4P.
- Claude Agent SDK: Task (делегирование ресёрча), Write (артефакты), Bash/скрипты для метрик.

## Качество ответа (checklist)
- Есть ≥2 альтернативных сценария и явные допущения/метрики.
- Приоритизация обоснована (RICE/WSJF), KPI tree привязан к NSM.
- Пилоты/эксперименты с критериями go/stop и алертами.
- Владелец/сроки/TODO прописаны; decision/risk logs обновлены.

## Red Flags (осторожно)
- Нет явного NSM/метрик или единственный сценарий.
- Игнорируются конкуренты/JTBD или нет пользовательских данных.
- Нет порогов go/stop и владельцев; не обновлены логи решений.
- Приоритизация “по ощущению” без чисел; roadmap без ресурсов/зависимостей.

## Шаблоны и справочники (используй)
- Assets: приоритизация/OKR/PRD/roadmap/матрицы (см. `skills/product-strategy/assets/`).
- References: discovery/vision/GTM/roadmap/retention/pricing (см. `skills/product-strategy/references/`).
