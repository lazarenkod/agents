...
- Сохраняй артефакты в `outputs/genai-architect/skills/production-rag-systems/{timestamp}_{кратко}.md` через Write tool; обновляй один файл по итерациям.
- Формат: цель/контекст → диагностика → архитектура/решения → метрики/алерты → тесты/безопасность → TODO → изменения vs прошлой версии.

## 3-итерационный контур
1) **Диагностика (1–2 ч):** требования к качеству/latency/стоимости/безопасности, домен/данные/PII, текущая архитектура/боли, объем/скорость обновлений. Черновой бриф + risk/decision log.
2) **Дизайн (2–4 ч):** варианты RAG (chunking/embeddings/store/rerank/generation), архитектура (ингест/индексация/кеш/obs), eval-план (offline/online), guardrails (safety/PII/policy), метрики/алерты/SLO. Таблицы решений/компонентов/владельцев.
3) **Верификация (1–2 ч):** PoC/пилоты, тесты/DR/rollback, алерты/пороги, план релиза/обслуживания, обновление логов/TODO/изменений.
...
- Домен/данные/PII, требования к latency/стоимости/качеству, частота обновлений, источники (доки/БД/форматы), требования к цитатам/ответственности.
- Ограничения: бюджет/модель/инфра, политики безопасности/регуляторика, масштабы нагрузки.

## Выходы (обязательно зафиксировать)
- Архитектура RAG (ингест/индексация/ранжирование/генерация/кеши/obs) и выбранные варианты.
- Eval-план (offline/online), метрики/алерты/SLO, guardrails (safety/PII/policy), runbooks (обновления/DR/rollback).
- TODO/владельцы/сроки, decision/risk log, изменения vs прошлой версии.

## Метрики и алерты
- Качество: groundedness/faithfulness, цитируемость, поисковая точность/recall, rerank качество.
- Производительность: latency p50/p95/p99, throughput/qps, cache hit rate, индексация/refresh время.
- Стоимость: $/ответ, $/ингест/хранение, egress.
- Надёжность/безопасность: error rate, timeout, policy/safety violations, PII/редакция.
- Алерты: рост latency/cost, падение groundedness/accuracy, рост policy violations/PII, деградация индекса/кеша.

## Качество ответа (checklist)
- Требования/ограничения собраны; архитектура покрывает ingest→search→rerank→generate→guardrails→obs.
- Eval-план offline/online с метриками/порогами; SLO/алерты определены.
- Runbooks/обновления/DR/rollback описаны; decision/risk logs и TODO обновлены.

## Red Flags
- Нет eval/регрессий; нет guardrails/PII/политик; нет кешей/obs.
- Не учтены частота обновлений/latency/data drift; один сценарий без альтернатив.
