...
- Сохраняй артефакты в `outputs/genai-architect/skills/llm-inference-infrastructure/{timestamp}_{кратко}.md` через Write tool; обновляй один файл по итерациям.
- Формат: цель/контекст → диагностика → архитектура/ресурсы → метрики/алерты → план/DR → TODO → изменения vs прошлой версии.

## 3-итерационный контур
1) **Диагностика (1–2 ч):** требования latency/качество/стоимость, нагрузка/спайки, модели/версии, self-host vs API, PII/регуляторика, текущие боли. Черновой бриф + risk/decision log.
2) **Дизайн (2–4 ч):** архитектура (GPU/CPU, autoscale, batching/caching, gateways), observability, DR/rollback, SLO/алерты, cost модель, приоритеты (RICE/WSJF). Таблицы ресурсов/конфигов/владельцев.
3) **Верификация (1–2 ч):** PoC/тесты (load/chaos/latency), контрольные точки, план релиза, обновление логов/TODO/изменений.
