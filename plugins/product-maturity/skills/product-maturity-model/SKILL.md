...
- Сохраняй артефакты в `outputs/product-maturity/skills/product-maturity-model/{timestamp}_{кратко}.md` через Write tool (обновляй один файл по итерациям).
- Формат: цель/контекст → диагностика → анализ уровней → выводы/приоритеты → план/алерты → TODO → изменения vs прошлой версии.

## 3-итерационный контур
1) **Диагностика (2–4 ч):** собрать вводные, провести self-assessment по измерениям и уровням, зафиксировать текущие метрики/проблемы/риски. Черновой бриф + decision/risk log.
2) **Дизайн (4–8 ч):** выбрать целевой уровень и 3–5 приоритетных capability, определить метрики/таргеты, инициативы/инвестиции, RICE/WSJF приоритет, алерты/пороги. Таблицы уровней/разрывов/планов.
3) **Верификация (1–2 ч):** согласовать go/no-go по инициативам, контрольные точки/ритм обзоров, обновить логи/план и TODO, зафиксировать изменения vs предыдущей версии.

## Когда применять
- Оценка зрелости продукта/организации и выстраивание плана роста.
- Подготовка к Board/ExCo/ревью портфеля по зрелости.
- Решение, куда инвестировать (процессы, платформа, культура, данные).
- Сравнение с бенчмарками FAANG/лидерами рынка.
...
- Текущие метрики: продуктовые (NSM, AARRR/HEART), инж (DORA/SLO), культура/HR (attrition, eNPS), финансы (CAC/LTV/payback, GM).
- Процессы и артефакты: roadmap/OKR, discovery/эксперименты, CI/CD, on-call, incident/RCA, документация.
- Оргструктура/роли, платформенные практики, риски/ограничения.

## Выходы (обязательно зафиксировать)
- Self-assessment по 6 измерениям и 5 уровням, ключевые разрывы.
- Приоритетный план (3–5 capability) с метриками/целями и алертами.
- Ритм обзоров (WBR/MBR/QBR) и контрольные точки, TODO/владельцы/сроки.
- Decision/Risk log, изменения vs прошлой версии.

## Метрики и алерты (примеры)
- Продукт: NSM, AARRR/HEART, adoption/retention, win rate.
- Инженерия: DORA, SLO/SLA, дефекты/краши, MTTR, debt ratio.
- Операции: deployment freq/lead time, incident rate, cost-to-serve.
- Культура: eNPS/attrition, найм/онбординг скорость, участие в RCA/ретро.
- Алерты: деградация SLO/качества, рост debt/attrition, невыполнение контрольных точек.

## Чек-лист качества оценки
- Чётко указаны уровень “как есть” и “куда” по каждому измерению.
- Привязаны метрики/таргеты и алерты; RICE/WSJF для инициатив.
- Определены владельцы, сроки, ритм обзоров; логи обновлены.
- Зафиксированы допущения/риски и изменения vs прошлой версии.

## Red Flags
- Отсутствуют метрики или опора только на субъективное мнение.
- Нет приоритетов/фокуса (слишком много инициатив).
- Нет владельцев/сроков/ритма; алерты/порогов go/stop не заданы.
- Нет связи с бизнес-целями/NSM или с операционными практиками.

## Глубокая оценка по измерениям (используй при разборе)
- **Стратегия/продукт:** видение/OKR/roadmap, discovery/validation, PMF сигналы, позиционирование/конкуренты, GTM связка.
- **Инжиниринг/платформа:** CI/CD, IaC, observability, SRE/SLO, архитектура/надёжность, безопасность, качество/тесты.
- **Культура/команда:** лидерские принципы, псих. безопасность, обратная связь, найм/онбординг, обучение, владение (DRI), ритуалы.
- **Процессы/операции:** incident/RCA, релизы/фич-флаги, change/risk mgmt, документация, операционный ритм (WBR/MBR/QBR).
- **Данные/аналитика:** доступность метрик/дашбордов, data quality, экспериментирование, ML/AI применение, governance/PII.
- **Финансы/бизнес:** CAC/LTV/payback, GM, бюджет/ресурсы, портфель инвестиций, unit economics по сегментам.

## Выводы и план (структура артефакта)
```markdown
# Оценка зрелости продукта/организации
**Дата:** {timestamp} | **Этап:** Диагностика/Дизайн/Верификация

## Контекст и цели
- Бизнес-цели/NSM, рынок/продукт, почему сейчас

## Self-assessment по измерениям
- Таблица: измерение → уровень (1–5) → факты/метрики → риски

## Разрывы и приоритеты
- Топ-5 разрывов (impact/effort), RICE/WSJF, зависимости

## План и метрики
- 3–5 инициатив: цели, владельцы, сроки, метрики/алерты, контрольные точки/ритм

## Риски/допущения
- Пороги stop/go, что может помешать
