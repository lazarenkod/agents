...
- Сохраняй артефакты в `outputs/fstec-security-analysis/skills/security-analysis-methodology/{timestamp}_{кратко}.md` через Write tool; обновляй один файл по итерациям.
- Формат: цель/контекст → диагностика/объём → требования (приказ 118 ФСТЭК/угрозы) → план/контроли → метрики/алерты → выводы/TODO → изменения vs прошлой версии.

## 3-итерационный контур
1) **Диагностика (1–2 ч):** контекст (класс/категория КИИ, ПДн, регуляторика), scope, архитектура/ОС/СЗИ, актуальная модель угроз ФСТЭК/117/187, требования приказа 118. Черновой бриф + risk/decision log.
2) **Дизайн/оценка (2–4 ч):** чек-листы контролей (118/стек угроз), тесты/аудит (конфиг/код/инфра), карта рисков и несоответствий, приоритеты remediation, план проверок/артефактов. Таблицы контролей/статуса/владельцев.
3) **Верификация (1–2 ч):** согласование мер, план внедрения/тестов, контрольные точки, финальный отчёт, обновление логов/TODO/изменений.
...
- Регуляторика: приказ 118 ФСТЭК, 17/21, КИИ/ПДн, внутренние стандарты.
- Архитектура: платформа (K8s/Docker/Podman), версии, окружения, компоненты (control/worker), СЗИ/СУБД/СУИ.
- Модель угроз ФСТЭК/актуализация, перечень активов/данных, список контуров/сетей.

## Выходы (обязательно зафиксировать)
- Матрица контролей/несоответствий vs 118 ФСТЭК и модель угроз.
- План remediation (меры/сроки/владельцы/приоритет), алерты/пороги, артефакты аудита.
- Decision/Risk log, TODO, изменения vs прошлой версии.

## Метрики и алерты
- % выполненных контролей/закрытых несоответствий, время remediation, повторные нарушения.
- Coverage: какие компоненты/окружения закрыты проверками, свежесть аудита.
- Алерты: критичные несоответствия/PII/КИИ, просроченные remediation, дрейф конфигов.

## Глубокие проверки (включай явно)
- **Управление доступом:** RBAC/ABAC, MFA, ротация ключей/сертификатов, минимальные привилегии, break-glass.
- **Сеть:** сегментация/NP/firewall, шифрование каналов (mTLS/IPsec), ограничения egress, контроль публичных точек.
- **Данные:** шифрование at rest/in transit, PII контроль, бэкапы/целостность, доступ к снапшотам.
- **CI/CD/supply chain:** подпись/скан образов, политика зависимостей, защита секретов, изоляция пайплайнов.
- **Логирование/мониторинг:** централизованный сбор, целостность логов, синхронизация времени, SIEM/SOAR.
- **Уязвимости/патчи:** регулярные сканы образов/узлов, SLA фиксов, контроль дрейфа IaC.
- **IR/BCP/DR:** планы реагирования, обучение, тесты, RTO/RPO, каналы эскалации.
- **Обоснование соответствия:** к каждому пункту 118 — ссылка на контроль, артефакт, измеряемая метрика, периодичность проверки.
- **Сэмплирование/оценка рисков:** критерии отбора выборок (образы/узлы/пайплайны), шкалы impact/вероятности, единый scoring для приоритизации remediation.
- **Сертифицированные ОС (RedOS 7/8):** использование сертифицированных репозиториев/пакетов, hardened ядро, включение SELinux, системный аудит, сертифицированная криптография (ViPNet/КриптоПро), контроль обновлений и хэшей, checklist узла.
- **Облака с сертификацией ФСТЭК:** изоляция тенантов/сегментов, доверенная виртуализация (гипервизор/СЗИ), криптозащита каналов/хранилищ, управление ключами, журналирование/СКУД, процедуры аттестации и контроль конфигураций.
- **Среда виртуализации:** защищённые гипервизоры/ВМ, изоляция tenants/VM, запрет несертифицированных модулей, контроль side-channel, шифрование storage/VM images, безопасные снапшоты/миграции, защита API/консолей, журналирование админских действий, соответствие приказам 17/21 и методикам по защите виртуализации.

## Качество ответа (checklist)
- Приказ 118/модель угроз учтены; scope полный; риски и несоответствия задокументированы.
- Есть план мер с приоритетами/сроками/владельцами, алерты/контрольные точки.
- Артефакты аудита/проверок перечислены; логи/TODO обновлены.

## Red Flags
- Нет привязки к 118/угрозам; нет списка активов/контуров.
- Нет статуса по контролям, только общие рекомендации.
- Нет плана remediation/алертов/владельцев; нет фиксации изменений.
...
- [Чек-лист RedOS 7/8](assets/templates/redos-baseline-checklist.md)
- [Чек-лист готовности облака к сертификации](assets/templates/cloud-cert-readiness.md)
- [Чек-лист безопасности виртуализации](assets/templates/virtualization-audit-checklist.md)

### Инструкции
- [Установка инструментов](references/tools-installation.md)
- [Настройка окружения](references/environment-setup.md)
- [Troubleshooting](references/troubleshooting.md)
- [Hardening RedOS 7/8](references/redos-hardening.md)
- [Требования ФСТЭК к сертифицированным облакам](references/fstec-cloud-certification.md)
- [Требования к защите виртуализации](references/fstec-virtualization-guidelines.md)
