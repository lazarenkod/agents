...
- Сохраняй артефакты в `outputs/cloud-provider-cto/skills/cloud-economics/{timestamp}_{кратко}.md` через Write tool; обновляй один файл по итерациям.
- Формат: цель/контекст → диагностика → анализ затрат/цен → решения/оптимизации → метрики/алерты → TODO → изменения vs прошлой версии.

## 3-итерационный контур
1) **Диагностика (1–2 ч):** сбор затрат (compute/storage/network/support), договоры/дисконты, теги/учёт, профиль нагрузок, бизнес-цели (GM/margin/payback). Черновой бриф + risk/decision log.
2) **Дизайн (2–4 ч):** сценарии оптимизаций (rightsizing/RI-SP/кластер/сторедж/egress), финмодель base/upside/downside, приоритизация (RICE/WSJF), метрики/алерты, политика бюджетов/квот. Таблицы экономии/действий/владельцев.
3) **Верификация (1–2 ч):** согласовать план/квоты, алерты/стоп-пороги, контрольные точки, обновить логи/TODO/изменения.
...
- Текущие затраты по сервисам/проектам/тегам, договоры (RI/SP/EA), egress/PII/региональные требования.
- Метрики бизнеса: GM/маржа, CAC/LTV/payback, прогноз спроса, важные сроки (финквартал/контракты).

## Выходы (обязательно зафиксировать)
- Финмодель (base/upside/downside), план оптимизаций с оценкой экономии и сроками.
- Политики бюджетов/квот, алерты, chargeback/showback модель, TODO/владельцы/даты.
- Decision/Risk log, изменения vs прошлой версии.

## Метрики и алерты
- $/unit (запрос/GB/пользователь), COGS/маржа, egress %, утилизация, RI/SP покрытие, idle/overprovision.
- Алерты: выход за бюджет, рост egress/PII, нагрузка > прогноз, истекающие скидки/контракты.

## Качество ответа (checklist)
- Полные входы (затраты/договоры/прогноз/PII), сценарии с цифрами и приоритизацией.
- Метрики/алерты и владельцы заданы; chargeback/showback продуман.
- План оптимизаций с оценкой impact/сроков; логи/TODO обновлены.

## Red Flags
- Нет тегирования/разделения затрат; неизвестны egress/PII риски.
- Одна опция без сценариев; нет приоритизации/метрик/алертов.
- Не учтены контракты/скидки/сроки; нет владельцев/дат.

## Шаблоны и справочники
- Assets: `cost-optimization-plan.md`, `fin-model-template.md`, `budget-policy-template.md`.
- References: `cost-metrics-map.md`, `optimization-playbook.md`, `contract-checklist.md`.
